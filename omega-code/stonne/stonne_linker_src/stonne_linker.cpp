...
void simulateSparseDenseGemm(std::string layer_name, float* MK_sparse_matrix, float* KN_dense_matrix, float* output_raw, int M, int K, int N, unsigned int* MK_col_id, unsigned int* MK_row_pointer, int T_N, int T_K, Config stonne_cfg) {
    ...
    std::string layer_name_batch=layer_name+"_batch_"+std::to_string(0);
    //Running STONNE
    Stonne* stonne_instance = new Stonne(stonne_cfg);

    stonne_instance->loadSparseDense(layer_name_batch, N, K, M, MK_sparse_matrix, KN_dense_matrix, MK_col_id, MK_row_pointer, output_raw, T_N, T_K ); //Loading GEMM
    stonne_instance->run(); //Running the simulator
    delete stonne_instance;
