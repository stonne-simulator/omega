...
//--------------- DSNetworkStats implementation ---------------
// ----------------------------------------------------------

DSNetworkStats::DSNetworkStats() {
    this->reset();
}

void DSNetworkStats::reset() {
    this->n_broadcasts=0;         //N times the Aswitches send a broadcast
    this->n_unicasts=0;           // N times the ASwitches send a unicast
    this->n_left_sends=0;         // N times the ASwitches send a message through the left port
    this->n_right_sends=0;        // N times the ASwitches send a message though the right port
    this->n_total_traversals=0;   // left+right sends

}

void DSNetworkStats::print(std::ofstream& out, unsigned int indent) {
    out << ind(indent) << "\"N_broadcasts\" : " << this->n_broadcasts << "," << std::endl;
    out << ind(indent) << "\"N_unicasts\" : " << this->n_unicasts << "," << std::endl;
    out << ind(indent) << "\"N_left_sends\" : " << this->n_left_sends << "," << std::endl;
    out << ind(indent) << "\"N_right_sends\" : " << this->n_right_sends << "," << std::endl;
    out << ind(indent) << "\"N_total_traversals\" : " << this->n_total_traversals  << std::endl;
}
...
//--------------- MSNetwork stats implementation ---------------
// ----------------------------------------------------------
...
void MSNetworkStats::reset() {
    this->n_multiplications=0;
    this->n_local_network_traversals=0;
    this->n_l1_weight_reads=0;
    this->n_l1_weight_writes=0;
    this->n_l1_input_reads=0;
    this->n_l1_input_writes=0;
    this->n_l1_psum_reads=0;
    this->n_l1_psum_writes=0;
}

void MSNetworkStats::print(std::ofstream& out, unsigned int indent) {
    out << ind(indent) << "\"N_multiplications\" : " << this->n_multiplications << "," << std::endl;
    out << ind(indent) << "\"N_local_network_traversals\" : " << this->n_local_network_traversals << "," << std::endl; //calculated statistic
    out << ind(indent) << "\"N_l1_weight_reads\" : " << this->n_l1_weight_reads << "," << std::endl; //calculated statistic
    out << ind(indent) << "\"N_l1_weight_writes\" : " << this->n_l1_weight_writes << "," << std::endl;
    out << ind(indent) << "\"N_l1_input_reads\" : " << this->n_l1_input_reads << "," << std::endl;
    out << ind(indent) << "\"N_l1_input_writes\" : " << this->n_l1_input_writes << "," << std::endl;
    out << ind(indent) << "\"N_l1_psum_reads\" : " << this->n_l1_psum_reads << "," << std::endl;
    out << ind(indent) << "\"N_l1_psum_writes\" : " << this->n_l1_psum_writes  << std::endl;  //Take care. We do not print the comma here.
...
//--------------- ASNetworkStats implementation ---------------
// ----------------------------------------------------------

ASNetworkStats::ASNetworkStats() {
    this->reset();
}

void ASNetworkStats::reset() {
    this->n_2_1_sums;                               //N cycles the ASwitches perform a 2_1 sum
    this->n_3_1_sums;                               //N cycles the ASwitches perform a 3_1 sum
    this->n_parent_send;                            //N cycles the ASwitches send a data to the parent
    this->n_augmented_link_send;                    //N cycles the ASwitches send a data to the augmented link
    this->n_memory_send;                            //N cycles the ASwitches send a data directly to memory (to the buses)
    this->n_total_traversals;                       // N times the links are being used
    this->n_total_acc_buffer_sums;                  //N sums performed by accumulation buffers

}

void ASNetworkStats::print(std::ofstream& out, unsigned int indent) {
    out << ind(indent) << "\"N_2_1_sums\" : " << this->n_2_1_sums << "," << std::endl;
    out << ind(indent) << "\"N_3_1_sums\" : " << this->n_3_1_sums << "," << std::endl;
    out << ind(indent) << "\"N_parent_send\" : " << this->n_parent_send << "," << std::endl;
    out << ind(indent) << "\"N_augmentendLink_send\" : " << this->n_augmented_link_send << "," << std::endl;
    out << ind(indent) << "\"N_memory_send\" : " << this->n_memory_send  << "," << std::endl;
    out << ind(indent) << "\"N_total_traversals\" : " << this->n_total_traversals  << "," << std::endl;
    out << ind(indent) << "\"N_total_acc_buffer_sums\" : " << this->n_total_acc_buffer_sums  << std::endl;  //Take care. We do not print the comma here. Last one
...
//--------------- AccumulationBufferStats implementation ---------------
// ----------------------------------------------------------

AccumulationBufferStats::AccumulationBufferStats() {
    this->reset();
}

void AccumulationBufferStats::reset() {
    this->n_adds=0;
    this->n_memory_send=0;
    this->n_receives=0;
    this->n_register_reads=0;
...
void AccumulationBufferStats::print(std::ofstream& out, unsigned int indent) {
    out << ind(indent) << "\"N_adds\" : " << this->n_adds << "," << std::endl; //calculated statistic
    out << ind(indent) << "\"N_memory_send\" : " << this->n_memory_send << "," << std::endl;
    out << ind(indent) << "\"N_receives\" : " << this->n_receives << "," << std::endl;
    out << ind(indent) << "\"N_register_reads\" : " << this->n_register_reads << "," << std::endl;
