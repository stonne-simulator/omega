...
void Stonne::loadDenseGEMM(std::string layer_name, unsigned int N, unsigned int K, unsigned int M, address_t MK_matrix, address_t KN_matrix, address_t output_matrix, Dataflow dataflow) {
    ...
    //input_matrix=KN
    //filter_matrix = MK
    loadDNNLayer(GEMM, layer_name, 1, K, 1, M, 1, 1, N, K, 1, KN_matrix, MK_matrix, output_matrix, dataflow);
    std::cout << "Loading a GEMM into STONNE" << std::endl;
}
...
void Stonne::loadClocking(unsigned int* clocked_op)
{
	this->mem->setClocking(clocked_op);
}


void Stonne::loadGEMMTile(unsigned int T_N, unsigned int T_K, unsigned int T_M)  {
    //loadTile(1, T_K, 1, T_M, 1, T_N, 1, 1);
    loadTile(1, T_K, 1, T_M, 1, 1, T_N, 1);
...
SDMemoryStats Stonne::getMemoryStats() {
    return this->mem->getStats();
}

MSNetworkStats Stonne::getMultiplierNetworkStats() {
    return this->msnet->getStats();
}

ASNetworkStats Stonne::getASNetworkStats() {
    return this->asnet->getStats();
}

DSNetworkStats Stonne::getDSNetworkStats() {
    return this->dsnet->getStats();
}
