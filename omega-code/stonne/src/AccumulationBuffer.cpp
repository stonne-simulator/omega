...
AccumulationBufferStats AccumulationBuffer::getStats() {
    AccumulationBufferStats accumulationbufferStats;
    //Collecting accumulator stats
    for(std::map<int,Accumulator*>::iterator it=accumulatortable.begin(); it != accumulatortable.end(); ++it) {
        Accumulator* acc = accumulatortable[it->first]; //index
	AccumulatorStats accumulatorStats = acc->getStats();
        accumulationbufferStats.n_adds+=accumulatorStats.n_adds;                                  
	accumulationbufferStats.n_memory_send+=accumulatorStats.n_memory_send;
        accumulationbufferStats.n_receives+=accumulatorStats.n_receives;
        accumulationbufferStats.n_register_reads+=accumulatorStats.n_register_reads;                         
	accumulationbufferStats.n_register_writes+=accumulatorStats.n_register_writes;
	accumulationbufferStats.n_configurations+=accumulatorStats.n_configurations;
     }

    return accumulationbufferStats;
}
