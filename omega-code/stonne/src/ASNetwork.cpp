...
ASNetworkStats ASNetwork::getStats() {
     ASNetworkStats asnetworkStats;
    //Iterating over Adders
    //
     for(std::map<std::pair<int,int>,ASwitch*>::iterator it=aswitchtable.begin(); it != aswitchtable.end(); ++it) {
        ASwitch* as = aswitchtable[it->first]; //Pair index
        ASwitchStats aswitchStats = as->getStats();
	asnetworkStats.n_2_1_sums+=aswitchStats.n_2_1_sums;
	asnetworkStats.n_3_1_sums+=aswitchStats.n_3_1_sums;
	asnetworkStats.n_parent_send+=aswitchStats.n_parent_send;
	asnetworkStats.n_augmented_link_send+=aswitchStats.n_augmented_link_send;
    ...
    for(std::map<std::pair<int, int>, Connection*>::iterator it=inputconnectiontable.begin(); it != inputconnectiontable.end(); ++it) {
	 ...
	 asnetworkStats.n_total_traversals+=conn_stats.n_sends;
     }
    ...
    for(std::map<std::pair<int, int>, Connection*>::iterator it=forwardingconnectiontable.begin(); it != forwardingconnectiontable.end(); ++it) {
     ...
     //AccumulationBuffer stats
     if(this->accumulation_buffer_enabled > 0) {
         asnetworkStats.n_total_acc_buffer_sums+=this->accumulationBuffer->getStats().n_adds;
	 asnetworkStats.n_memory_send+=this->accumulationBuffer->getStats().n_memory_send;
     }
    //this->n_total_acc_buffer_sums;                  //N sums performed by accumulation buffers
    return asnetworkStats;
