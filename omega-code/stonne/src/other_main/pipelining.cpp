...
int main()
{
	...
	T_Va=18;T_N=1;T_Fa=28;T_Vc=18;T_G=1;T_Fc=28;
	int T_Vp, T_Fp;
	...
	if(granularity==ROW)
	{
		...
		std::ifstream MK_mat("eval_set/adj_mutag_batch64.txt");
...
std::cout << "Running CPU version to compare results" << std::endl;
   ...
   MSNetworkStats mulstats = stonne_instance2->getMultiplierNetworkStats();
	...
	int* clk2 = new int[V/T_Va+(V%T_Va!=0)];		////////This becomes ceil(V/T_Va)	
		...
		int l_cycles=(V/T_Vc+(V%T_Vc!=0))*(F/T_Fc+(F%T_Fc!=0));
		std::cout<<"\n\nL Cycles = "<<l_cycles<<"\n\n";
		int sp_buffers = G*T_Vc*(F/T_Fc+(F%T_Fc!=0));
		int p_cycles = ((sp_buffers/pe_cmb)+(sp_buffers%pe_cmb!=0))*(V/T_Vc+(V%T_Vc!=0));
		if(F==T_Fc)
		p_cycles=0;
		...
		////////////Printing stats and analytical model here
