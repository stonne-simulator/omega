...
void configSparseGEMMParameters(int argc, char *argv[], Config &stonne_cfg, std::string &layer_name, unsigned int &M, unsigned int &N, unsigned int &K, unsigned int &MK_sparsity, unsigned int &KN_sparsity, Dataflow &dataflow);
...
bool runSparseGEMMCommand(int argc, char *argv[]) {
    ...
    configSparseGEMMParameters(argc, argv, stonne_cfg, layer_name, M, N, K, MK_sparsity, KN_sparsity, dataflow);

    //Creating MK matrix
    float* MK_dense_matrix = generateMatrixDense(M, K, MK_sparsity);
    //Creating KN_matrix
    float* KN_dense_matrix = generateMatrixDense(K, N, KN_sparsity);
    ...
    */
    ...
    cpu_gemm(MK_dense_matrix, KN_dense_matrix, cpu_output, M, N, K);
