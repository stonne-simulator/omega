...
std::string get_string_memory_hierarchy_type(MemoryHierarchy_t memory_hierarchy_type) {
    switch(memory_hierarchy_type) {
        case DOUBLE_BUFFER:
...
MemoryHierarchy_t get_type_memory_hierarchy_type(std::string memory_hierarchy_type) {
        if(memory_hierarchy_type=="DOUBLE_BUFFER") {
            return DOUBLE_BUFFER;
        }
        else if(memory_hierarchy_type=="BUFFET") {
            return BUFFET;
        }
        else {
            std::cout << memory_hierarchy_type << " Not found" << std::endl;
            assert(false);
        }
...
float* generateMatrixDense(unsigned int rows, unsigned int cols, float sparsity) {
    float* matrix = new float[rows*cols];
    for(int i=0; i<rows; i++) {
        for(int j=0; j<cols; j++) {
	    float value;
            float sparse_yes=float(rand())/(float(RAND_MAX)+1.0)*100.0;
...
float* generateMatrixDenseSampled(unsigned int rows, unsigned int cols, int nbr) {
    float* matrix = new float[rows*cols];
    float sparsity=80.0;
    for(int i=0; i<rows; i++) {
	int count=0;
	    ...
	    float value;
            float sparse_yes=float(rand())/(float(RAND_MAX)+1.0)*100.0;
	    if(sparse_yes <= sparsity || count>=nbr) {
                value=0.0;
	    }
