...
DSNetworkStats DSNetwork::getStats() {
    DSNetworkStats dsnetworkStats;
    //Collecting DSwitch stats
     for(std::map<std::pair<int, int>, DSwitch*>::iterator it=dswitchtable.begin(); it != dswitchtable.end(); ++it) {
        DSwitch* ds = dswitchtable[it->first];
        DSwitchStats dswitchStats = ds->getStats();
        dsnetworkStats.n_broadcasts+=dswitchStats.n_broadcasts;
	dsnetworkStats.n_unicasts+=dswitchStats.n_unicasts;
	dsnetworkStats.n_right_sends+=dswitchStats.n_right_sends;
	dsnetworkStats.n_left_sends+=dswitchStats.n_left_sends;
