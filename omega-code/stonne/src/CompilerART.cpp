...
void CompilerART::configureSignals(Tile* current_tile, DNNLayer* dnn_layer, unsigned int num_ms, unsigned int n_folding) {
    ...
    this->generate_art_signals(num_ms);
    this->generate_art_enabling_links(num_ms);
    
}

void CompilerART::configureSparseSignals(std::vector<SparseVN> sparseVNs, DNNLayer* dnn_layer, unsigned int num_ms) {
    assert(ispowerof2(num_ms));
    ...
    this->signals_configured = true;
    //Configuring ART switches
    this->generate_art_signals_sparse(num_ms);
    this->generate_art_enabling_links_sparse(num_ms);
   ...
   for(int i=0; i<this->current_tile->get_Num_VNs(); i++) { //For each neuron
	...
	//std::cout << std::endl;
      ...
      //                         std::cout << "  FW link is not enabled so the configuration is done by this other process" << std::endl;
            ...
            if(n_bits_enabled==1) { //If there is just one enabled
                ...
                forwarding_to_memory_enabled[switch_to_memory_pair]=true;
               // break;
...
// Los siguientes niveles son recorridos mirando abajo y mirando en la configuracion del nivel inferior. Si tiene hijo 
 //   std::cout << "Generating ART Enabling links signals" << std::endl;
    ...
    for(int i=0; i<this->sparseVNs.size(); i++) {
        ms_used+=this->sparseVNs[i].get_VN_Size();
    }
    int num_adders=num_ms / 2; //ALl the adders
    //Last level (not included the ms) apart since we just must look at the ms in order to see if the link is enabled
    for(int i=0; i<num_adders; i++) {
