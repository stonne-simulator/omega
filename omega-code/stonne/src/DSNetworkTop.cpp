...
DSNetworkStats DSNetworkTop::getStats() {
     DSNetworkStats dsnetworkStats;
     //Wires between memory and DSNetwork
      for(int i=0; i<this->n_input_ports; i++)  {
           ConnectionStats conn_stats = this->connections[i]->getStats();
	   dsnetworkStats.n_total_traversals+=conn_stats.n_sends;
      }

      //Adding the ASNetworks
      for(int i=0; i<this->n_input_ports; i++)  {
          DSNetworkStats current_stats = this->dsnetworks[i]->getStats();
          dsnetworkStats.n_broadcasts+=current_stats.n_broadcasts;
          dsnetworkStats.n_unicasts+=current_stats.n_unicasts;
          dsnetworkStats.n_right_sends+=current_stats.n_right_sends;
          dsnetworkStats.n_left_sends+=current_stats.n_left_sends;
	  dsnetworkStats.n_total_traversals+=current_stats.n_total_traversals;
      }

     return dsnetworkStats;
