...
void Config::loadFile(std::string config_file) {
    ...
    auto memory_hierarchy_type_conf = config->get_qualified_as<std::string>("SDMemory.memory_hierarchy_type");
    if(memory_hierarchy_type_conf) {
        this->m_SDMemoryCfg.mem_hierarchy_type = get_type_memory_hierarchy_type(*memory_hierarchy_type_conf);
    }

    auto sdmemory_l1_sram_weight_size_conf = config->get_qualified_as<unsigned int>("SDMemory.l1_sram_weight_size");
    if(sdmemory_l1_sram_weight_size_conf) {
        this->m_SDMemoryCfg.l1_sram_weight_size = *sdmemory_l1_sram_weight_size_conf;
    }

     auto sdmemory_l1_sram_input_size_conf = config->get_qualified_as<unsigned int>("SDMemory.l1_sram_input_size");
    if(sdmemory_l1_sram_input_size_conf) {
        this->m_SDMemoryCfg.l1_sram_input_size = *sdmemory_l1_sram_input_size_conf;
    }

     auto sdmemory_l1_sram_output_size_conf = config->get_qualified_as<unsigned int>("SDMemory.l1_sram_output_size");
    if(sdmemory_l1_sram_output_size_conf) {
        this->m_SDMemoryCfg.l1_sram_output_size = *sdmemory_l1_sram_output_size_conf;
    }

    auto sdmemory_l1_sram_weight_latency_conf = config->get_qualified_as<unsigned int>("SDMemory.l1_sram_weight_latency");
    if(sdmemory_l1_sram_weight_latency_conf) {
        this->m_SDMemoryCfg.l1_sram_weight_latency = *sdmemory_l1_sram_weight_latency_conf;
    }

     auto sdmemory_l1_sram_input_latency_conf = config->get_qualified_as<unsigned int>("SDMemory.l1_sram_input_latency");
    if(sdmemory_l1_sram_input_latency_conf) {
        this->m_SDMemoryCfg.l1_sram_input_latency = *sdmemory_l1_sram_input_latency_conf;
    }

     auto sdmemory_l1_sram_output_latency_conf = config->get_qualified_as<unsigned int>("SDMemory.l1_sram_output_latency");
    if(sdmemory_l1_sram_output_latency_conf) {
        this->m_SDMemoryCfg.l1_sram_output_latency = *sdmemory_l1_sram_output_latency_conf;
    }
...
// ---------------------------------------------------------
    ...
    m_SDMemoryCfg.mem_hierarchy_type=BUFFET;
    m_SDMemoryCfg.l1_sram_weight_size=32; //32 KB
    m_SDMemoryCfg.l1_sram_input_size=32; //32 KB
    m_SDMemoryCfg.l1_sram_output_size=32; //32 KB
    m_SDMemoryCfg.l1_sram_weight_latency=1; //1 cycle
    m_SDMemoryCfg.l1_sram_input_latency=1; //1 cycle
    m_SDMemoryCfg.l1_sram_output_latency=1; //1 cycle
...
void SDMemoryConfig::printConfiguration(std::ofstream& out, unsigned int indent) {
	...
	out << ind(indent+IND_SIZE) << "\"mem_hierarchy_type\" : " << "\"" << get_string_memory_hierarchy_type(this->mem_hierarchy_type) << "\""  << "," << std::endl;
	out << ind(indent+IND_SIZE) << "\"l1_sram_weight_size\" : " << this->l1_sram_weight_size << "," << std::endl;
	out << ind(indent+IND_SIZE) << "\"l1_sram_input_size\" : " << this->l1_sram_input_size << "," << std::endl;
	out << ind(indent+IND_SIZE) << "\"l1_sram_output_size\" : " << this->l1_sram_output_size << "," << std::endl;
	out << ind(indent+IND_SIZE) << "\"l1_sram_weight_latency\" : " << this->l1_sram_weight_size << "," << std::endl;
        out << ind(indent+IND_SIZE) << "\"l1_sram_input_latency\" : " << this->l1_sram_input_size << "," << std::endl;
        out << ind(indent+IND_SIZE) << "\"l1_sram_output_latency\" : " << this->l1_sram_output_size << "," << std::endl;
        out << ind(indent+IND_SIZE) << "\"port_width\" : " << this->port_width << std::endl;
    out << ind(indent) << "}";
